...
class Classifier_CNNMI:
	...
	def build_model(self, inputA_shape, inputB_shape, nb_classes, n_feature_maps=64):
		...
		# B part, auxiliaries
		conv1B = keras.layers.Conv1D(filters=128, kernel_size=8, padding='same')(inputB_layer)
		conv1B = keras.layers.BatchNormalization()(conv1B)
		conv1B = keras.layers.Activation(activation='relu')(conv1B)

		pooling_layerB = keras.layers.GlobalAveragePooling1D()(conv1B)
		dense_layerB = keras.layers.Dense(64, activation='relu')(pooling_layerB)

		modelB = keras.models.Model(inputs=inputB_layer, outputs=dense_layerB)
	...
	def fit(self, x_trainAB, y_train, x_valAB, y_val,y_true):
		...
		np.save(self.output_directory + 'fcnMI_y_pred.npy', y_pred)
		np.save(self.output_directory + 'fcnMI_y_true.npy', y_true)

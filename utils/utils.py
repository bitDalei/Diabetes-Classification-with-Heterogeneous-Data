...
def addNoise(df):
    ...
    # Add random noise following the Gaussian distribution to the original array 
    ...
    tails = df_array[:,577:588]
    tails_new = addNoise_util(tails)
    df_new = np.hstack((df_array[:,0:577],tails_new))
    # pd.DataFrame(df_new).to_csv(root+'\\'+filename+'_way23.csv',header=None,index=None)

    return df_new

# Import datasets, undivided training and test sets, randomly divided
def load_dataset1(data_rootdir, filename,flag_addNoise=False):

    df = pd.read_csv(data_rootdir+filename,header=None)
    ...
    x_train_cgm = x_train[:,:-11]
    x_test_cgm = x_test[:,:-11]
    x_train_attr = x_train[:,-11:]
    x_test_attr = x_test[:,-11:] 

    return x_train_cgm, x_train_attr, x_test_cgm, x_test_attr, y_train, y_test

    # pd.DataFrame(df_new).to_csv(data_rootdir+'2.csv',header=None,index=None)

# Import the dataset, and the training and test sets have been divided
def load_dataset2(data_rootdir, filename1, filename2,flag_addNoise):

    df1 = pd.read_csv(data_rootdir+filename1,header=None)
    df2 = pd.read_csv(data_rootdir+filename2,header=None)

    if flag_addNoise == True:
        df1_new = addNoise(df1)
        df2_new = addNoise(df2)
        np.random.shuffle(df1_new)        
        np.random.shuffle(df2_new)
    else:
        df1_new = df1.to_numpy()
        df2_new = df2.to_numpy()
        np.random.shuffle(df1_new)
        np.random.shuffle(df2_new)
    ...
    x_train_cgm = df1_new[:,:-11]
    x_test_cgm = df2_new[:,:-11]
    x_train_attr = df1_new[:,-11:]
    x_test_attr = df2_new[:,-11:]
